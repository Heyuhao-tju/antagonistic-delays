...
def calc_R(alpha, k):
    ...
    def dist_x(x):
        ...
        if omega < 0 or omega > omega_max:
            return np.inf
        L_mu = (1 + 1j * omega * mu / 2.0) ** (-2)
        L_tau = (1 + 1j * omega * tau / 2.0) ** (-2)
...
ax.set_title(r'$R(\alpha; k)$ for several values of $k$ ($\Gamma$-kernel $n=2$)', fontsize=13)
...
plt.tight_layout()
plt.savefig('R_alpha_family_gamma.svg', format='svg', bbox_inches='tight', pad_inches=0.05)
plt.show()