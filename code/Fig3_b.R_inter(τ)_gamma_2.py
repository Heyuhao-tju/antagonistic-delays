...
def compute_R_inter(tau, n):
    """
    Self-delay kernel is fixed as Dirac (mu_fixed).
    Interaction-delay kernel: n=1,5,10,100 for Gamma; n=-1 for Dirac.
    """
    ...
    if tau <= 100:
        ...
        def dist_x(x):
            ...
            if omega < 0 or omega > 1e3:
                return np.inf
            L_self_local = L_dirac(omega, mu_fixed)
            L_inter_local = L_gamma(omega, tau, n)
            numerator = 1j * omega + d_A - D * L_self_local
            b = numerator / L_inter_local
            return np.abs(b)
...
n_inter_list = [1, 5, 10, 100, -1]

R_inter_curves = []
for n in n_inter_list:
    R_inter_curves.append(np.array([compute_R_inter(tau, n) for tau in tau_vals]))
...
plt.tight_layout()
plt.savefig('R_inter_repair_dirac_self.svg', format='svg', bbox_inches='tight')
plt.show()