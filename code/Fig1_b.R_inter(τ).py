...
# Compute the critical distance for pure self-delay at mu=10,
...
def calc_R_self(mu):
    ...
    # Coarse scan over the scaled variable x
...
def calc_R_inter(tau):
    def dist_x(x):
        ...
        if omega < 0 or omega > omega_max:
            return np.inf
        b = (1 + 1j * omega * tau) * (1j * omega + d_A - D / (1 + 1j * omega * mu_fixed))
        return abs(b)
...
ax.set_title(r'Stability recovery by interaction delay ($\mu=10$)', fontsize=13)
...
plt.tight_layout()
plt.savefig('R_inter.svg', format='svg', bbox_inches='tight', pad_inches=0.05)
plt.show()